...
public class ConsultaBean implements Serializable {
	...
	private String selectedEvent = "DDF025";
	...
	public void executeProgram() {
		try {
			...
			for (DadosPlanilha item : filteredData) {
				if (item.isExecutado()) {
			...
			// Se não houver nenhum item já executado, a execução prossegue
				R4020cnpj32243347000151.main(null, selectedEvent);
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Ocorreu um erro durante a execução.","Erro!");
			FacesContext.getCurrentInstance().addMessage("growl", message);
		}
	}
